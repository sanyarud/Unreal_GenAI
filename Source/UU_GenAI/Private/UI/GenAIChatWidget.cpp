    ...
    Request->OnProcessRequestComplete().BindLambda([this](FHttpRequestPtr Req, FHttpResponsePtr Res, bool bSuccess) {
        ...
        if (FJsonSerializer::Deserialize(Reader, JsonObj))
        {
            ...
            if (JsonObj->TryGetArrayField(TEXT("models"), ModelList))
            {
                ...
                {
                    if (Models[i].Group == TEXT("Ollama") && Models[i].ModelId != TEXT("sep"))
                    {
                ...
                if (InsertIdx == -1) InsertIdx = Models.Num();
                ...
                for (int32 i = 0; i < Models.Num(); ++i)
                {
                    ...
                    {
                        ComboItems.Add(MakeShared<FString>(TEXT("── ") + Models[i].Group + TEXT(" ──")));
...
static FSlateBrush MakeRoundedBrush(const FLinearColor& Color, float Radius = 8.f)
{
    FSlateBrush B;
    B.TintColor = FSlateColor(Color);
    B.DrawAs    = ESlateBrushDrawType::Box;
...
void SGenAIChatWidget::Construct(const FArguments& InArgs)
{
    ...
    Models = {
        ...
        { TEXT("─── Local ───"),       TEXT("Ollama"),    TEXT("ollama"),    TEXT("sep")                       },
        { TEXT("─── Custom ───"),     TEXT("None"),      TEXT("none"),      TEXT("sep")                       },
        ...
        [
            ...
            [
                ...
                [
                    ...
                    + SHorizontalBox::Slot().MaxWidth(900.f)
                    [ BuildScrollArea() ]
                    + SHorizontalBox::Slot().FillWidth(1.f)
                ]
                ...
                [
                    SNew(SHorizontalBox)
                    + SHorizontalBox::Slot().FillWidth(1.f)
                    + SHorizontalBox::Slot().MaxWidth(800.f)
                    [
                        SNew(SVerticalBox)
                        + SVerticalBox::Slot().AutoHeight()
                        [ BuildAttachmentBar() ]
...
TSharedRef<SWidget> SGenAIChatWidget::BuildHeader()
{
    ...
    const FSlateFontInfo FontSmall = FCoreStyle::GetDefaultFontStyle("Regular", 10);
    ...
    return SNew(SBorder)
        ...
        [
            ...
            [
                SAssignNew(CustomModelBox, SEditableTextBox)
                .HintText(FText::FromString(TEXT("Enter model ID...")))
                .Font(FontSmall)
                .MinDesiredWidth(140.f)
                .Visibility_Lambda([this]() {
                    const auto& InnerModels = GetModels();
                    if (InnerModels.IsValidIndex(SelectedModel) && InnerModels[SelectedModel].ModelId == TEXT("custom"))
...
TSharedRef<SWidget> SGenAIChatWidget::BuildScrollArea()
{
    return SNew(SBorder)
        ...
        [
            SAssignNew(MessagesScroll, SScrollBox)
            .ScrollBarVisibility(EVisibility::Collapsed)
            + SScrollBox::Slot()
            [
                SAssignNew(MessagesVBox, SVerticalBox)
            ]
        ];
}
...
TSharedRef<SWidget> SGenAIChatWidget::BuildAttachmentBar()
{
    ...
    TSharedPtr<SWidget> Bar =
        ...
        .Visibility_Lambda([this]() {
            return AttachmentName.IsEmpty() ? EVisibility::Collapsed : EVisibility::Visible;
        })
        [
            SNew(SHorizontalBox)
            + SHorizontalBox::Slot().AutoWidth().VAlign(VAlign_Center)
            ...
            + SHorizontalBox::Slot().AutoWidth().VAlign(VAlign_Center)
            [
                SAssignNew(AttachLabel, STextBlock)
                .Font(Font)
                .ColorAndOpacity(ChatPalette::Accent)
            ]
        ...
        [
            ...
            [
                ...
                [
                    ...
                    [
                        SNew(SBox).WidthOverride(24.f).HeightOverride(16.f).HAlign(HAlign_Center).VAlign(VAlign_Center)
                        [
                            SNew(STextBlock)
                            .Text(FText::FromString(TEXT("＋")))
                            .Font(Font)
                ...
                [
                    ...
                    [
                        ...
                        [
                            SNew(STextBlock).Text(FText::FromString(TEXT("💡"))).Font(Font)
                        ]
                        + SHorizontalBox::Slot().AutoWidth().VAlign(VAlign_Center)
                        [
                            SNew(STextBlock)
                ...
                [
                    ...
                    [
                        ...
                        [
                            ...
                            .OnSelectionChanged(this, &SGenAIChatWidget::OnModelSelected)
                            .OnGenerateWidget(this, &SGenAIChatWidget::MakeModelRow)
                            [
                                SNew(STextBlock)
                                .Text(this, &SGenAIChatWidget::GetCurrentModelLabel)
                                .Font(Font)
                                .ColorAndOpacity(ChatPalette::Text)
                            ]
                        ]
                ...
                [
                    ...
                    .IsEnabled_Lambda([this]() { return !bStreaming; })
                    [
                        SNew(SBox).WidthOverride(32.f).HeightOverride(32.f).HAlign(HAlign_Center).VAlign(VAlign_Center)
                        [
                            SNew(STextBlock)
                            .Text(FText::FromString(TEXT("↑")))
...
TSharedRef<SWidget> SGenAIChatWidget::MakeModelRow(TSharedPtr<FString> Item)
{
    ...
    const FSlateFontInfo Font = bIsSeparator
        ? FCoreStyle::GetDefaultFontStyle("Italic", 9)
        : FCoreStyle::GetDefaultFontStyle("Regular", 11);

    return SNew(SBox).Padding(FMargin(bIsSeparator ? 8.f : 14.f, 4.f))
        [
            SNew(STextBlock)
            .Text(FText::FromString(*Item))
            .Font(Font)
            .ColorAndOpacity(bIsSeparator ? ChatPalette::Subtext : ChatPalette::Text)
...
void SGenAIChatWidget::OnModelSelected(TSharedPtr<FString> Item, ESelectInfo::Type)
{
    if (!Item.IsValid() || Item->StartsWith(TEXT("──"))) return;
    const auto& FinalModels = GetModels();
...
TSharedRef<SWidget> SGenAIChatWidget::MakeBubble(bool bIsUser, const FString& Text)
{
    ...
    TSharedPtr<STextBlock> TB = TextBlock;

    if (!bIsUser) StreamingLabel = TB;

    TSharedRef<SWidget> Bubble =
        SNew(SBox).MaxDesiredWidth(460.f)
        [
            SNew(SBorder)
            .BorderImage(BubBrush)
            .Padding(FMargin(12.f, 8.f))
            [ TB.ToSharedRef() ]
        ];
    ...
    return SNew(SHorizontalBox).Clipping(EWidgetClipping::OnDemand)
        ...
        [
            ...
            + SVerticalBox::Slot().AutoHeight().Padding(FMargin(
                bIsUser ? 80.f : 0.f, 3.f,
                bIsUser ? 0.f   : 80.f, 3.f))
            [
                Bubble
            ]
        ];
...
{
    if (!MessagesVBox.IsValid()) return;
    StreamingLabel.Reset();

    MessagesVBox->AddSlot().AutoHeight().Padding(FMargin(0, 12, 0, 0))
    [
        SNew(SHorizontalBox)
        + SHorizontalBox::Slot().FillWidth(1.f)
        + SHorizontalBox::Slot().AutoWidth()
        [
            SNew(SBox).MaxDesiredWidth(500.f)
            [
                SNew(SBorder)
...
void SGenAIChatWidget::AppendAIBubble(const FString& Text, bool bIsCurrentlyStreaming)
{
    if (!MessagesVBox.IsValid()) return;

    TSharedPtr<STextBlock> Label;
    MessagesVBox->AddSlot().AutoHeight().Padding(FMargin(0, 24, 0, 0))
    [
        SNew(SVerticalBox)
        ...
        [
            ...
            + SHorizontalBox::Slot().AutoWidth()
            [
                SNew(SBox).MaxDesiredWidth(800.f)
                [
                    SAssignNew(Label, STextBlock)
                    .Text(FText::FromString(bIsCurrentlyStreaming ? TEXT("…") : Text))
                    .Font(FCoreStyle::GetDefaultFontStyle("Regular", 13)) // slightly larger
                    .ColorAndOpacity(ChatPalette::Text)
                    .AutoWrapText(true)
                ]
            ]
...
void SGenAIChatWidget::UpdateStreamingBubble(const FString& FullText, bool bDone)
{
    if (StreamingLabel.IsValid())
    {
        StreamingLabel->SetText(FText::FromString(bDone ? FullText : FullText + TEXT("▌")));
    }
    ScrollToBottom();
}
...
{
    if (!InputBox.IsValid() || bStreaming) return;
        ...
        if (bIsCoding)
        {
            ...
            for (int32 i = 0; i < FinalModels.Num(); ++i)
            {
                if (FinalModels[i].Provider == Settings->SmartRouting.CodingProvider &&
                   (FinalModels[i].ModelId.Contains(Settings->SmartRouting.CodingModelId) || FinalModels[i].ModelId == Settings->SmartRouting.CodingModelId))
                {
                    RoutingModelIdx = i;
                    break;
                }
...
void SGenAIChatWidget::OnInputCommitted(const FText& Text, ETextCommit::Type Type)
{
...
void SGenAIChatWidget::SendToProvider(const TArray<FGenChatMessage>& Messages, int32 ModelIdx)
{
    const auto& FinalModels = GetModels();
    if (!FinalModels.IsValidIndex(ModelIdx)) return;
    ...
    auto OnDelta = [SelfWeak](const FGenStreamDelta& Delta, bool bSuccess)
    {
        ...
        if (!bSuccess)
        {
            Self->UpdateStreamingBubble(TEXT("⚠ Request failed. Check your API key in Project Settings."), true);
    ...
    if (Provider == TEXT("openai"))
    {
        ...
        S.ModelString = ModelId;
        S.SystemPrompt = SkillsPrompt;
        S.MaxTokens   = 2048;
        UGenOAIChatStream::SendStreamChatRequest(Messages, S,
            FOnOAIStreamDeltaNative::CreateLambda(OnDelta));
    }
    else if (Provider == TEXT("anthropic"))
    {
        ...
        S.SystemPrompt = SkillsPrompt;
        S.MaxTokens   = 4096;
        UGenAnthropicChatStream::SendStreamChatRequest(Messages, S,
            FOnAnthropicStreamDeltaNative::CreateLambda(OnDelta));
    }
    else if (Provider == TEXT("google"))
    {
        ...
        UGenGoogleChat::SendStreamChatRequest(Messages, S,
            FOnGoogleStreamDeltaNative::CreateLambda(OnDelta));
    }
    else if (Provider == TEXT("deepseek"))
    {
        ...
        S.SystemPrompt = SkillsPrompt;
        S.MaxTokens   = 4096;
        UGenCompatChat::SendStreamDeepSeekRequest(Messages, S,
            FOnCompatStreamDeltaNative::CreateLambda(OnDelta));
    }
    else if (Provider == TEXT("mistral"))
    {
        ...
        S.SystemPrompt = SkillsPrompt;
        S.MaxTokens   = 4096;
        UGenCompatChat::SendStreamMistralRequest(Messages, S,
            FOnCompatStreamDeltaNative::CreateLambda(OnDelta));
    }
    else if (Provider == TEXT("ollama"))
    {
        ...
        S.ModelString = ModelId;
        S.SystemPrompt = SkillsPrompt;
        S.BaseUrl = GetDefault<UGenAISettings>()->OllamaBaseUrl;
        UGenOllamaChatStream::SendStreamChatRequest(Messages, S,
            FOnOllamaStreamDeltaNative::CreateLambda(OnDelta));
    }
}
...
#else
    UE_LOG(LogTemp, Warning, TEXT("[GenAI Chat] Blueprint attachment is only available in Editor."));
#endif
...
{
    if (!s_OverlaySlot.IsValid()) return;
...
bool UGenAIChatWidget::IsGenAIChatOpen()
{
    return s_OverlaySlot.IsValid();
}
